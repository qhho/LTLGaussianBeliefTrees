...
Scene::Scene(const std::string& scene_config)
{
    ...
    if (sceneConfig["scene"]["obstacles"].IsSequence()) {
        ...
        if (sceneConfig["scene"]["propositions]"].IsSequence()) {
        for (const auto& obstacle : sceneConfig["scene"]["propositions"]) {
            define_cube_as_constraints_regions(obstacle["fx"].as<double>(), obstacle["tx"].as<double>(), obstacle["fy"].as<double>(), obstacle["ty"].as<double>(), obstacle["fz"].as<double>(), obstacle["tz"].as<double>());
            n_regions_++;
        }
    }
...
void Scene::define_cube_as_constraints_regions(const double &fx, const double &tx, const double &fy, const double &ty, const double &fz, const double &tz){
    ...
    A_list_regions_.push_back(A);
    B_list_regions_.push_back(B);
}